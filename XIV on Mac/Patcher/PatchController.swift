...
class PatchController: NSViewController {
    ...
    let installQueue = DispatchQueue(
        label: "installer.serial.queue", qos: .utility, attributes: [],
        autoreleaseFrequency: .workItem)
    let patchQueue = DispatchQueue(
        label: "patch.installer.serial.queue", qos: .utility, attributes: [],
        autoreleaseFrequency: .workItem)
    ...
    func install(_ patches: [Patch]) {
        ...
        DispatchQueue.main.async { [self] in
            installPatch.stringValue = NSLocalizedString(
                "PATCH_INSTALLATION_WAITING", comment: "")
            installStatus.stringValue = String(
                format: NSLocalizedString(
                    "PATCH_INSTALLATION_PROGRESS_INIT", comment: ""),
                patches.count)
        ...
        for (patchNum, _) in patches.enumerated() {
            installQueue.async { [self] in
                install(
                    patchNum: patchNum, patches: patches, totalSize: totalSize)
            }
        }
    ...
    private func install(patchNum: Int, patches: [Patch], totalSize: Int64) {
        ...
        updateProgress(
            totalCompletedSize: partialSize + patch.length,
            totalSize: totalSize, completedSize: patch.length,
            size: patch.length, speed: 0)
        patchQueue.async {
            DispatchQueue.main.async { [self] in
            ...
            DispatchQueue.main.async { [self] in
                ...
                installStatus.stringValue = String(
                    format: NSLocalizedString(
                        "PATCH_INSTALLATION_PROGRESS", comment: ""),
                    installsDone, patches.count)
        ...
        do {
            try HTTPClient.fetchFile(
                ...
                self.updateProgress(
                    ...
                    totalSize: totalSize, completedSize: now, size: total,
                    speed: speed)
            }
        } catch {
                ...
                DispatchQueue.main.sync {
                    ...
                    alert.addButton(
                        withTitle: NSLocalizedString(
                            "PATCH_INSTALLATION_ERROR_BUTTON", comment: ""))
                    alert.alertStyle = .critical
                    alert.messageText = NSLocalizedString(
                        "PATCH_INSTALLATION_ERROR_MESSAGE", comment: "")
                    alert.informativeText = String(
                        format: NSLocalizedString(
                            "PATCH_INSTALLATION_ERROR_INFORMATIVE", comment: ""),
                        patch.url.absoluteString, maxTries)
                    alert.runModal()
                    Util.quit()
                }
                return
            ...
            download(
                patch, totalSize: totalSize, partialSize: partialSize,
                tries: tries + 1, maxTries: maxTries)
        }
    }

    private func updateProgress(
        totalCompletedSize: Int64, totalSize: Int64, completedSize: Int64,
        size: Int64, speed: Int64
    ) {
        func format(_ value: Int64) -> String {
            formatter.string(fromByteCount: value)
        }
        DispatchQueue.main.async { [self] in
            downloadStatus.stringValue = String(
                format: NSLocalizedString(
                    "PATCH_INSTALLATION_DOWNLOAD_STATUS", comment: ""),
                format(totalCompletedSize), format(totalSize))
            downloadPatchStatus.stringValue = String(
                format: NSLocalizedString(
                    "PATCH_INSTALLATION_DOWNLOAD_STATUS_PATCH", comment: ""),
                format(completedSize), format(size), format(speed))
            downloadBar.doubleValue =
                downloadBar.maxValue * Double(totalCompletedSize)
                / Double(totalSize)
            downloadPatchBar.doubleValue =
                downloadPatchBar.maxValue * Double(completedSize) / Double(size)
        }
    }
